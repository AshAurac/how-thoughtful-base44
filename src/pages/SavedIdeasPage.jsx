...
export default function SavedIdeasPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="font-accent text-ink-soft text-lg">things you loved</p>
        <h1 className="font-heading font-bold text-2xl text-ink">Saved Ideas</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="font-accent text-xl text-ink-soft">nothing saved yet</p>
          <p className="text-sm text-ink-soft mt-1">Bookmark ideas from the Ideas page to see them here.</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between mb-1">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-heading font-semibold text-ink">{idea.name}</h3>
                      {isFree && <span className="text-xs bg-moss/20 text-moss-dark px-2 py-0.5 rounded-full">free</span>}
                    </div>
                    {idea.recipient_name && (
                      <p className="text-xs text-ink-soft mt-0.5">For {idea.recipient_name}</p>
                    )}
                  </div>
                  <button
